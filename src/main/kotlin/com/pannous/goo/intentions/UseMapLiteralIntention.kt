...
class UseMapLiteralIntention : PsiElementBaseIntentionAction(), IntentionAction {
    
    override fun getText(): String = "Use Goo map literal syntax"
    ...
    override fun isAvailable(project: Project, editor: Editor?, element: PsiElement): Boolean {
        ...
        // Pattern: map[string]int{"key": value, ...}
    ...
    override fun invoke(project: Project, editor: Editor?, element: PsiElement) {
        ...
        // Pattern: map[keyType]valueType{"key": value, "key2": value2}
        val pattern = Pattern.compile(
            """map\[[^\]]+\]\s*\{([^}]*)\}""",
            Pattern.DOTALL
        )
        ...
        if (matcher.find()) {
            val mapContents = matcher.group(1)
            
            // Convert "key": value to key: value (remove quotes from string keys)
            val simplifiedContents = mapContents
                .replace(Regex(""""([^"]+)"\s*:"""), "$1:")
                .trim()
            
            val replacement = line.replace(matcher.group(0), "{$simplifiedContents}")
